...
public class Block2SpeakingIntentionController : MonoBehaviour
{
    ...
    public void StartTrial(int trialNumber)
    {
        ...
        if (!trials.ContainsKey(trialNumber))
        {
            Debug.LogWarning("Block2 trial " + trialNumber + " is not configured.");
            return;
        }
    ...
    private bool CanRunHere()
    {
        if (!IsControlledScene(SceneManager.GetActiveScene()))
        {
            Debug.LogWarning("Block2SpeakingIntentionController only runs in " + controlledSceneName + ".");
            return false;
        }
        ...
        if (!HasTargets())
        {
            Debug.LogWarning("Block2SpeakingIntentionController could not find ZHZ, DCY, and ZJR SpeakingIntention targets.");
            return false;
        }
    ...
    private void BuildBlock2Schedules()
    {
        trials.Clear();
        trials.Add(1, new[]
        {
