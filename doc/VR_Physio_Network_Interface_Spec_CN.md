...
| interfaceMode | 建议 UI 行为 |
|---|---|
| realtime_or_rich | 可以显示较明确的实时 speaking-intention cue |
| subtle_realtime | 显示轻量 peripheral cue 或低强度 cue |
| subtle_or_delayed | 降低即时反馈强度，必要时推迟到 summary |
| summary_or_fallback | 不依赖生理状态，只使用普通 summary 或默认界面 |
| subtle_or_summary | 使用保守策略 |
